...
class Corrector(object):
    ...
    def __list_test_files(self):
        files = glob.glob('.\\tests\\*.in')
        files.sort()
        return files
    
    def __compile(self):
        out = subprocess.Popen(['make'], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, shell=True)
    ...
    def __compare_outputs(self, file1, file2):
        with open(file1, 'r') as f1, open(file2, 'r') as f2:
            data_file1 = f1.read().replace(' ', '').replace('\n', '')
            data_file2 = f2.read().replace(' ', '').replace('\n', '')
    ...
    def __check_response(self, input_test_cases):
        ...
        for input_test_case in input_test_cases:
            ...
            if self.__compare_outputs('output.out', input_test_case.replace('.in', '.out')):
                ...
                with open(input_test_case.replace('.in', '.out'), 'r') as f1, open('output.out', 'r') as f2:
                    data_file1 = f1.read()
                    data_file2 = f2.read()
